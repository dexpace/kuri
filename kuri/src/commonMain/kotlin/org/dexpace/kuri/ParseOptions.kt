 ...
 * regardless of any option ([HOST-17]). This keeps the `Url` profile unconditionally
 * WHATWG-conformant.
 *
 ...
 * ([CONF-126]). A `ParseOptions` is a value type — equality and hashing are structural over its
 ...
 * through `toString` then `parse` ([HOST-18]).
 */
public class ParseOptions private constructor(
     ...
     * ([HOST-18]); honored by the **`Uri` profile only**.
     *
     * `false` by default: with the opt-in off, an IPv6 literal carrying a `%` is rejected ([HOST-17]).
    ...
    public class Builder {
        ...
        /**
         * Enables or disables RFC 6874 IPv6 zone-id parsing ([HOST-18]); off by default.
         *
         * @param allow `true` to accept a `%25`-introduced zone id, `false` to reject it ([HOST-17]).
