...
source (Unicode 16.0.0, shipped under .claude/references) and emits a
...
# The Unicode version this generator targets. Bump together with SOURCE_TXT so the
# generated KDoc stays accurate and reproducible.
UNICODE_VERSION = "16.0.0"

REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
# Default source: the bundled Unicode 16.0.0 IdnaMappingTable.txt. Override on the
# command line (`generate_idna_mapping_table.py [INPUT_TXT [OUTPUT_KT]]`) for a
# different Unicode release without editing this file.
SOURCE_TXT = os.path.join(
    REPO_ROOT,
    ".claude", "references", "unicode-16.0", "IdnaMappingTable.txt",
)
...
def render_kotlin(chunks):
    lines = [
        ...
        "",
        "/**",
        " * Compact, generated UTS-46 IDNA mapping table data (Unicode %s)." % UNICODE_VERSION,
        " *",
...
def main():
    source_txt = sys.argv[1] if len(sys.argv) > 1 else SOURCE_TXT
    output_kt = sys.argv[2] if len(sys.argv) > 2 else OUTPUT_KT
